...
- Active user-reported regressions are acceptance evidence: preserve each report verbatim in its own detail subsection under `docs/development/ISSUES.md`; do not edit, collapse, or remove it while fixing; validate against the original bullets and examples.
...
- Tests and validation evidence are grounded in immutable requirement/acceptance ids and user-visible behavior, not task ids. Never name test files, test titles, assertion tags, fixtures, or helper identifiers after a task number.
...
- A vendor's or framework's own start command is not that launch path and never substitutes for it: its defaults bind fixed ports and one shared instance, so running it inside a worktree silently rejoins the lane the isolation exists to separate and corrupts the neighbour's data.
...
- `$reconcile` classifies feedback as strategic requirement reconsideration, operational design reconsideration, planner rescope, implementation defect, validation gap, or status-assessment gap.
...
- A defect whose cause lives outside this repo stays open here until the owning fix ships. Its detail section carries the reproduce/observed/expected/root-cause writeup, and that writeup **is** the hand-off: it travels to the owner as a file, never as a bare id reference.
