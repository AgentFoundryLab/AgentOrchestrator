...
**Namespace grammar (`--namespace` input)**: single dash-notation token matching `[a-z][a-z0-9-]*` (example: `orchestrator-core`). This is an installer input grammar, not a claim that every runtime supports dash identifiers at invocation time.

**Translation rule**:
- Commands for runtimes with native command namespacing (Gemini, Qwen custom commands): use runtime-native namespace-by-path and `:` invocation (e.g., `commands/<ns>/<cmd>` -> `/<ns>:<cmd>`).
- Commands for runtimes without native command namespacing: use dash fallback naming (`<ns>-<cmd>`).
- Skills/subagents for flat-naming runtimes (Claude project/user skills, Codex, OpenCode, Qwen skills/subagents): use dash fallback naming when `--namespace` is provided (`<ns>-<name>`).
...
Explicit `runtime × mode × key → action` rules beyond the minimal strip. Covers runtime-specific extensions (Codex invocation field and Gemini TOML structural requirements beyond basic format).
...
### Alternative for D-2: Suppress namespace for all non-native-namespace runtimes
